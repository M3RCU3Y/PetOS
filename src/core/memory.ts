...
export class PetMemory {
  ...
  constructor(seed?: { memories?: EpisodicMemory[]; surfacePreferences?: Record<string, number>; appPreferences?: Record<string, number>; toyPreferences?: Record<string, number>; relationships?: Record<string, number | Relationship> }) {
    ...
    if (seed?.relationships) {
      for (const [id, value] of Object.entries(seed.relationships)) {
        this.relationships.set(id, typeof value === "number" ? { ...blankRelationship(), familiarity: value } : { ...blankRelationship(), ...value });
      }
    }
  }
  ...
  relate(id: string): Relationship {
    let rel = this.relationships.get(id);
    if (!rel) { rel = blankRelationship(); this.relationships.set(id, rel); }
    return rel;
  }

  noteEncounter(id: string, kind: SocialEncounterKind): void {
    const rel = this.relate(id);
    const bump = (key: keyof Relationship, d: number) => { rel[key] = clamp(rel[key] + d, -1, 1); };
    switch (kind) {
      case "greet": bump("familiarity", .03); bump("trust", .01); break;
      case "play": bump("familiarity", .04); bump("affection", .025); bump("trust", .02); bump("rivalry", -.01); break;
      case "fight": bump("familiarity", .03); bump("rivalry", .035); bump("irritation", .015); break;
      case "cuddle": case "share": bump("affection", .04); bump("trust", .03); bump("familiarity", .03); bump("irritation", -.025); break;
      case "steal": bump("irritation", .04); bump("rivalry", .05); break;
    }
  }

  adjustRelationship(id: string, delta: number): void {
    const rel = this.relate(id);
    rel.affection = clamp(rel.affection + delta, -1, 1);
    rel.trust = clamp(rel.trust + delta * .6, -1, 1);
    rel.familiarity = clamp(rel.familiarity + Math.abs(delta) * .5, -1, 1);
  }
  ...
  /** Derived single value for UI and legacy scoring: positive = friendly, negative = hostile. */
  relationshipWith(id: string): number {
    const rel = this.relationships.get(id);
    if (!rel) return 0;
    return clamp(rel.familiarity*.18 + rel.trust*.32 + rel.affection*.38 - rel.irritation*.28 - rel.rivalry*.22, -1, 1);
  }
