...
export class PetBrain {
  ...
  decide(state:PetState,world:WorldSnapshot,memory:PetMemory):Decision{
    ...
    const nearestPet=[...world.nearbyPets].sort((x,y)=>x.distance-y.distance)[0];
    if(nearestPet){
      const rel=memory.relate(nearestPet.id);
      if(nearestPet.distance<320&&rel.familiarity<.18)add("investigate",d.curiosity*.3+p.curiosity*.22+.06,"curious about the newcomer",{id:nearestPet.id,position:nearestPet.position});
      ...
      if(nearestPet.distance<450){
        ...
        if(nearestPet.distance<140){
          ...
          if(nearestPet.behavior==="sleep"&&rel.trust>.18&&d.fatigue>.4)add("cuddle",d.fatigue*.34+rel.affection*.32+rel.trust*.18,"sleeps piled beside a trusted friend",{id:nearestPet.id,position:nearestPet.position});
          if(p.playfulness>.55&&d.play>.45&&(rel.rivalry>.25||d.play>.75))add("play_fight",d.play*.32+rel.rivalry*.24+p.playfulness*.2,"wrestles with a sparring partner",{id:nearestPet.id,position:nearestPet.position});
        }
      }
    }
    ...
    if(this.seeking){
      ...
      if(found)this.seeking=null;
        ...
        if(arrived&&world.nowMs-state.behaviorSinceMs>1500){
          ...
          if(missCount>=3)this.seeking=null;
          else{
    ...
    if(!food&&d.hunger>.6&&this.objects.knowsAbout("bowl")){
      ...
      }else if(remembered&&this.objects.gaveUp(remembered.id,world.nowMs))state.frustration=clamp(state.frustration+.02);
    }
    ...
    if(bed&&d.fatigue>.55){
      ...
      if(occupant){
        ...
        if(rel.rivalry>.35||rel.irritation>.4)add("play_fight",d.fatigue*.18+rel.rivalry*.32+p.boldness*.14,"contests the occupied bed",{id:occupant.id,position:{...bed.position}});
        else add("sleep",d.fatigue*.9+rel.trust*.16+(bed.comfort??.7)*.2,"shares a warm bed with a friend",{id:bed.id,position:bed.position});
      }else add("sleep",d.fatigue*.96+(bed.comfort??.7)*.25,"comfortable bed available",{id:bed.id,position:bed.position});
    }
    ...
    const favoriteId=state.favoriteSurfaceId??memory.favoriteSurface();
    if(favoriteId&&favoriteId!==current?.id){
      const favorite=world.surfaces.find(s=>s.id===favoriteId);
      if(favorite){
        const pref=memory.preferenceForSurface(favorite.id);
        if(pref>.15){
          const target={x:favorite.rect.x+favorite.rect.width/2,y:favorite.walkY};
          const distanceToFavorite=Math.hypot(state.body.position.x-target.x,state.body.position.y-target.y);
          const habit=clamp(pref*.36+state.habitStrength*.16+calm*.08,0,.62);
          if(distanceToFavorite>70){
            add("walk",habit+d.fatigue*.12+(1-d.comfort)*.08,"returns to a learned favorite spot",{id:favorite.id,position:target});
          }else{
            add("sit",habit+.16+d.comfort*.08,"settles back into a favorite spot",{id:favorite.id,position:target});
            if(d.fatigue>.42)add("sleep",habit+d.fatigue*.48,"returns to a trusted favorite sleeping spot",{id:favorite.id,position:target});
          }
        }
      }
    }
    ...
    if(current){
      ...
      if(state.species==="cat"||state.species==="bird"){
        const above=world.surfaces.filter(s=>s.kind==="window"&&s.rect.height>70&&(current.walkY-s.walkY)>60&&(current.walkY-s.walkY)<320&&!(state.body.position.x<s.rect.x-40||state.body.position.x>s.rect.x+s.rect.width+40));
        const best=above[0];
        if(best){
      ...
      if(frights.length&&pref<.3)add("walk",.12+frights.length*.05,"uneasy about this spot",{position:{x:world.cursor.position.x,y:current.walkY}});
    }
