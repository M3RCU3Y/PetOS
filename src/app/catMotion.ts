...
function drawWithKinetics(c:CanvasRenderingContext2D,p:PetState,a:PetAppearance,pose:IllustratedCatPose,t:number,artT:number,state:MotionState,alpha=1):void{
  ...
  drawPose(c,p,a,pose,artT);c.restore();
}

/** Smooth world/transition timing with an optional held-frame clock for the painted cat. */
export function drawIllustratedCat(c:CanvasRenderingContext2D,p:PetState,a:PetAppearance,target:IllustratedCatPose,t:number,reduceMotion=false,artT=t):void{
  if(reduceMotion||systemReducedMotion()){drawPose(c,p,a,target,artT);return;}
  const sig=signature(p,target);let state=states.get(p.id);
  if(!state){const initialFamily=family(p,target);state={signature:sig,family:initialFamily,fromFamily:initialFamily,from:target,last:target,startedAt:t,seenAt:t,lastGrounded:p.body.grounded,landingAt:-Infinity};states.set(p.id,state);drawPose(c,p,a,target,artT);return;}
  ...
  if(progress>=1){drawWithKinetics(c,p,a,target,t,artT,state);state.last=target;}
  else if(sameFamily){const visual=blend(state.from,target,progress);drawWithKinetics(c,p,a,visual,t,artT,state);state.last=visual;}
  else{
    const swap=.46;
    if(linear<swap){const q=smoothstep(linear/swap);c.save();c.translate(0,q*2.6);c.scale(1+q*.07,1-q*.17);drawWithKinetics(c,p,a,state.from,t,artT,state,1-q*.08);c.restore();state.last=state.from;}
    else{const q=smoothstep((linear-swap)/(1-swap));c.save();c.translate(0,(1-q)*2.6);c.scale(1+(1-q)*.07,.83+(q*.17));drawWithKinetics(c,p,a,target,t,artT,state,.92+q*.08);c.restore();state.last=target;}
  }
