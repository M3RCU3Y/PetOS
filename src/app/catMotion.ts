...
function drawWithKinetics(c:CanvasRenderingContext2D,p:PetState,a:PetAppearance,pose:IllustratedCatPose,t:number,state:MotionState,alpha=1):void{
  ...
  c.globalAlpha*=alpha;
  if(landing>0){
    // Contact has weight: compress on the first frame, then rebound into the pose.
    c.translate(0,landing*1.8);
    c.scale(1+landing*.105,1-landing*.12);
  }
  if(lean>0)c.rotate(lean);
  drawPose(c,p,a,pose,t);
  c.restore();
}

/**
 * Presentation-only pose compositor. The simulation changes behavior instantly;
 * this layer gives those decisions anticipation, contact and recovery without
...
export function drawIllustratedCat(c:CanvasRenderingContext2D,p:PetState,a:PetAppearance,target:IllustratedCatPose,t:number):void{
  ...
  if(!state){
    state={signature:sig,from:target,last:target,startedAt:t,seenAt:t,lastGrounded:p.body.grounded,landingAt:-Infinity};
    states.set(p.id,state);drawPose(c,p,a,target,t);return;
  }

  state.seenAt=t;
  if(!state.lastGrounded&&p.body.grounded)state.landingAt=t;
  state.lastGrounded=p.body.grounded;
  ...
  const sameFamily=family(state.from)===family(target);
  const duration=sameFamily?SAME_FAMILY_MS:SILHOUETTE_MS;
  const linear=clamp((t-state.startedAt)/duration);
  const progress=eased(linear);

  if(progress>=1){
    drawWithKinetics(c,p,a,target,t,state);
    state.last=target;
  }else if(sameFamily){
    const visual=blend(state.from,target,progress);
    drawWithKinetics(c,p,a,visual,t,state);
    state.last=visual;
  }else{
    // Do not ghost two cats together. The old silhouette compresses into the floor,
    // swaps near the midpoint, then the new silhouette softly springs to full height.
    const swap=.46;
    if(linear<swap){
      const q=smoothstep(linear/swap);
      c.save();c.translate(0,q*2.6);c.scale(1+q*.07,1-q*.17);
      drawWithKinetics(c,p,a,state.from,t,state,1-q*.08);c.restore();
      state.last=state.from;
    }else{
      const q=smoothstep((linear-swap)/(1-swap));
      c.save();c.translate(0,(1-q)*2.6);c.scale(1+(1-q)*.07,.83+(q*.17));
      drawWithKinetics(c,p,a,target,t,state,.92+q*.08);c.restore();
      state.last=target;
    }
  }
