...
function drawSitting(c:CanvasRenderingContext2D,p:PetState,a:PetAppearance,pose:IllustratedCatPose,t:number,m:CatMorph):void{
  ...
  if(markingFor(a)==="tabby")line(c,rgba(pal.deep,.5),1.4,()=>{for(let i=0;i<3;i++){c.moveTo(-10+i*6,-31);c.quadraticCurveTo(-7+i*6,-26,-6+i*6,-21.5);}});
...
function drawSleeping(c:CanvasRenderingContext2D,p:PetState,a:PetAppearance,pose:IllustratedCatPose,t:number,m:CatMorph):void{
  ...
  ellipse(c,7,-7,13,4.6,rgba(a.accent,.42),-.12);if(markingFor(a)==="tabby")line(c,rgba(pal.deep,.48),1.5,()=>{for(let i=0;i<4;i++){const x=-14+i*7;c.moveTo(x,-19);c.quadraticCurveTo(x+2,-15,x+4,-12);}});
...
function drawLoaf(c:CanvasRenderingContext2D,a:PetAppearance,pose:IllustratedCatPose,t:number,m:CatMorph):void{
  ...
  ellipse(c,6,-17,12,3.8,rgba(pal.hi,.22),-.04);if(markingFor(a)==="tabby")line(c,rgba(pal.deep,.5),1.5,()=>{for(let i=0;i<4;i++){const x=-13+i*7;c.moveTo(x,-20);c.quadraticCurveTo(x+2,-16,x+4,-12);}});else if(markingFor(a)==="tuxedo")ellipse(c,9,-5,9.4,4,rgba(a.accent,.8),-.05);else if(markingFor(a)==="patched")ellipse(c,-8,-13,6.8,4.9,rgba(a.accent,.52),-.2);
