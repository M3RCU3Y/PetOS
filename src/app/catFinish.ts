...
type Anchor={x:number;y:number;rotate?:number};
...
function headAnchor(p:PetState,pose:IllustratedCatPose):Anchor|null{
  if(pose.peeking)return{x:0,y:-10};
  if(pose.hanging)return{x:1,y:9};
  if(pose.pouncing&&!p.body.grounded)return null;
  if(pose.loaf)return{x:15,y:-20+Math.sin(performance.now()/1700)*.3};
...
function faceFinish(c:CanvasRenderingContext2D,p:PetState,a:PetAppearance,pose:IllustratedCatPose):void{
  const anchor=headAnchor(p,pose);if(!anchor)return;
  const {x,y}=anchor,pattern=a.markings??"tabby",deep=shade(a.coat,.47),variant=hash01(`${p.id}:face-mark`);
  if(pattern==="tabby"){
    // A tiny readable forehead M and cheek bars survive the final low-res pass well.
    ...
    // Some tuxedos get a narrow blaze rather than a perfectly symmetrical mask.
    ...
    const side=variant>.5?1:-1;
    ellipse(c,x+side*5.7,y-5.3,2.45,2.8,rgba(a.accent,.38),side*.2);
  }
  // Nose catchlight and lower-lip pixel make the face read at desktop scale.
...
function bodyFinish(c:CanvasRenderingContext2D,p:PetState,a:PetAppearance,pose:IllustratedCatPose):void{
  ...
  if(!pose.lying&&!pose.loaf&&!pose.sitting){
    ...
    // Sparse guard-hair glints break up large generated gradients without making fur noisy.
    line(c,"rgba(255,244,224,.18)",.55,()=>{c.moveTo(-10,bodyY-2.4);c.lineTo(-5.8,bodyY-3.2);c.moveTo(-1.5,bodyY-3.3);c.lineTo(3,bodyY-3.8);});
    if(pattern==="tabby"&&variant>.32){
      line(c,rgba(deep,.44),.85,()=>{c.moveTo(-18,bodyY+2);c.lineTo(-15,bodyY+5.3);c.moveTo(-15,bodyY+1.2);c.lineTo(-12.2,bodyY+4.6);});
    }
    if(pattern==="tuxedo"){
      // A soft bib edge behind the front legs makes the tuxedo pattern read in motion.
      ellipse(c,14.3,bodyY+10.2,5.1,3.05,rgba(a.accent,.34),-.12);
    }
  }else if(pose.loaf){
...
export function drawCatFinish(c:CanvasRenderingContext2D,p:PetState,a:PetAppearance,pose:IllustratedCatPose):void{
  bodyFinish(c,p,a,pose);faceFinish(c,p,a,pose);
}
