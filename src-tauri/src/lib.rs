...
fn get_desktop_snapshot(state: tauri::State<'_, DesktopState>) -> DesktopSnapshot {
    ...
    let activity = if raw.fullscreen { "fullscreen" } else if cursor_speed > 20.0 { "active" } else { "idle" };
    DesktopSnapshot { monitors:raw.monitors,windows:raw.windows,cursor:raw.cursor,cursor_speed,cursor_buttons:raw.cursor_buttons,foreground_app:raw.foreground_app,user_activity:activity.into(),seconds_since_new_window:since }
}
...
mod platform {
    ...
    pub struct Raw {pub monitors:Vec<MonitorInfo>,pub windows:Vec<DesktopWindow>,pub cursor:Point,pub cursor_buttons:u32,pub foreground_app:Option<String>,pub fullscreen:bool}
    ...
    pub fn snapshot()->Raw{unsafe{let mut monitors=Vec::new();EnumDisplayMonitors(0,std::ptr::null(),monitor_cb,&mut monitors as *mut _ as LPARAM);let mut windows=Vec::new();EnumWindows(window_cb,&mut windows as *mut _ as LPARAM);let fg=GetForegroundWindow();for w in &mut windows{w.foreground=w.id==format!("hwnd:{fg}");}let mut p=POINT{x:0,y:0};GetCursorPos(&mut p);let cursor=Point{x:p.x as f64,y:p.y as f64};let mut buttons=0;if GetAsyncKeyState(1)<0{buttons|=1}if GetAsyncKeyState(2)<0{buttons|=2}let fg_app=windows.iter().find(|w|w.foreground).map(|w|w.app.clone()).filter(|s|!s.is_empty());let fullscreen=windows.iter().find(|w|w.foreground).map(|w|monitors.iter().any(|m|w.rect.width>=m.rect.width*.97&&w.rect.height>=m.rect.height*.94)).unwrap_or(false);Raw{monitors,windows,cursor,cursor_buttons:buttons,foreground_app:fg_app,fullscreen}}}
...
mod platform {
    use super::{DesktopWindow,MonitorInfo,Point,Rect};
    pub struct Raw {pub monitors:Vec<MonitorInfo>,pub windows:Vec<DesktopWindow>,pub cursor:Point,pub cursor_buttons:u32,pub foreground_app:Option<String>,pub fullscreen:bool}
    pub fn snapshot()->Raw{Raw{monitors:vec![MonitorInfo{id:"primary".into(),rect:Rect{x:0.0,y:0.0,width:1280.0,height:800.0},work_area:Rect{x:0.0,y:0.0,width:1280.0,height:760.0},primary:true,scale_factor:1.0}],windows:vec![],cursor:Point{x:640.0,y:400.0},cursor_buttons:0,foreground_app:None,fullscreen:false}}
