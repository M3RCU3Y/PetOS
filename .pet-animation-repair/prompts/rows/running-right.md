Create one horizontal animation strip for Codex pet `petos-cozy-cats`, state `running-right`.
...
Identity: same pet in every frame: Keep the existing compact three-quarter-view cat identity and detailed pixel rendering. Rebuild only temporally coherent animation strips.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `pixel`: Pixel-art-adjacent digital mascot with a chunky silhouette, simple dark outline, limited palette, flat cel shading, and visible stepped edges. User style notes: Strict fixed camera, fixed scale, fixed foot baseline, no whole-sprite warping, no shadows or effects..
