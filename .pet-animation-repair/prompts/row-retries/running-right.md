Create Codex pet row `running-right` for `petos-cozy-cats`: exactly 8 full-body frames in one horizontal strip on flat pure cyan #00FFFF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Keep the existing compact three-quarter-view cat identity and detailed pixel rendering. Rebuild only temporally coherent animation strips.. Preserve silhouette, face, palette, material, proportions, markings, and props.
